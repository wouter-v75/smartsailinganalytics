...
export async function POST(req: NextRequest) {
  ...
  // A `prompt` field is deliberately NOT forwarded. A glossary prompt sent Scaleway's
  // whisper into repetition loops in every 5-minute chunk of the Maxi Worlds 2026
  // debriefs and lost whole topics; the summariser applies the vocabulary instead.

  const body = new FormData()
  body.append('file', file, name)
  body.append('model', MODEL)
  // Only pin the language when the caller gave a real code (not empty / "auto").
  if (language && language.toLowerCase() !== 'auto') body.append('language', language)
