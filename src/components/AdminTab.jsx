  ...
  return (
    <>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
        ...
        ].map(c=>(<div key={c.title} style={{background:"#0A1929",border:"1px solid #1E3A5A",borderRadius:10,padding:14}}><div style={{fontSize:11,fontWeight:600,color:"#8A97A9",marginBottom:8}}>{c.title}</div>{c.items.map((item,i)=><div key={i} style={{fontSize:10,color:"#4E5D71",padding:"3px 0",borderBottom:"1px solid #0F2030"}}>{item}</div>)}</div>))}
      </div>
      ...
      <div style={{background:"#0A1929",border:"1px solid #EF444430",borderRadius:10,padding:14}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          ...
          }} style={{background:"#0F2030",border:"1px solid #1E3A5A",borderRadius:6,padding:"7px 14px",color:"#8A97A9",cursor:"pointer",fontSize:11}}>
            📊 Storage info
          </button>
        </div>
      </div>
